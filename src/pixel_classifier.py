...
def save_predictions(args, image_paths, preds):
    ...
    for i, pred in enumerate(preds):
        ...
        print("saves the predictions")
        print("path:")
        print(os.path.join(args['exp_dir'], 'visualizations', filename + '.jpg'))
        np.save(os.path.join(args['exp_dir'], 'predictions', filename + '.npy'), pred[0])

        pred = np.squeeze(pred)
        mask = colorize_mask(args["in_out_channels"], pred, palette)
...
def compute_dice(args, preds, gts, print_per_class_ious=True):
    ...
    for target_num in ids:
        if target_num == args['ignore_label']:
            continue
        iou = (2 * intersections[target_num]) / ((1e-8 + unions[target_num]) + intersections[target_num])
        ious.append(iou)
        if print_per_class_ious:
            print(f"IOU for {class_names[target_num]} {iou:.4}")
    return np.array(ious).mean()

def compute_SSIM(args, preds, gts, print_per_class_ious=True):
    ssim_value = 1 - torch.ssim(args, preds).item()
    ssim_loss = torch._ssim.SSIM()
