...
def make_transform(model_type: str, resolution: int):
    ...
    if model_type == 'ddpm':
        transform = transforms.Compose([
            transforms.Resize(resolution), # resizes the images to the requested resolution
            transforms.ToTensor(),
            lambda x: 2 * x - 1
        ])
...
class ImageLabelDataset(Dataset): # Inherits from torch Dataset superclass
    ''' 
    ...
    def __init__(
        ...
        self.resolution = resolution # number of pixels in an image
        ...
        ] # For each image path take the part before .png and change it to npy to get the label path
    ...
    def __getitem__(self, idx):
        # Load an image
        image_path = self.image_paths[idx] # Get the image path of the requested index
        pil_image = Image.open(image_path) # Open the image as a PIL image

        if self.in_out_channels==3:
            pil_image = pil_image.convert("RGB") # Convert image to RGB
        else:
            pil_image = pil_image.convert()

        assert pil_image.size[0] == pil_image.size[1], \
               f"Only square images are supported: ({pil_image.size[0]}, {pil_image.size[1]})" # Makes sure
        # the channels of the images are of the same size

        tensor_image = self.transform(pil_image) # Transforms the images according to the saved transformations

        # Load a corresponding mask and resize it to (self.resolution, self.resolution)
        label_path = self.label_paths[idx] # Get the label path of the requested index
        label = np.load(label_path).astype('uint8') # Transform the label values to integers
        label = cv2.resize(
            label, (self.resolution, self.resolution), interpolation=cv2.INTER_NEAREST
        ) # Make sure that the label images and the actual images are of the same size
        tensor_label = torch.from_numpy(label) # Turn the np matrix into a torch tensor
